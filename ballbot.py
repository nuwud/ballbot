...
gyroRolllRaw  = open("ev3devices/ev3-ports:in3/value0", "rb")
...
# Open sensor files for (fast) reading
motorEncoderRolll1 = open("ev3devices/ev3-ports:outA/position", "rb")   
motorEncoderPitch1 = open("ev3devices/ev3-ports:outB/position", "rb")   
motorEncoderRolll2 = open("ev3devices/ev3-ports:outC/position", "rb")   
motorEncoderPitch2 = open("ev3devices/ev3-ports:outD/position", "rb")    
     
# Open motor files for (fast) writing
motorDutyCycleRolll1 = open("ev3devices/ev3-ports:outA/duty_cycle_sp", "w")
motorDutyCyclePitch1 = open("ev3devices/ev3-ports:outB/duty_cycle_sp", "w")
motorDutyCycleRolll2 = open("ev3devices/ev3-ports:outC/duty_cycle_sp", "w")
...
while True:
    ...
    motorAngleRawRolll              = 0 # The angle of "the motor", measured in raw units (degrees for the EV3). We will take the average of both motor positions as "the motor" angle, which is essentially how far the middle of the robot has traveled.
    ...
    motorAngularSpeedReferenceRolll = 0 # The reference speed during maneuvers: how fast we would like to drive, measured in radians per second.
    motorAngularSpeedErrorRolll     = 0 # The error: the deviation of the motor speed from the reference speed.
    motorDutyCycleRolll             = 0 # The 'voltage' signal we send to the motor. We calculate a new value each time, just right to keep the robot upright.
